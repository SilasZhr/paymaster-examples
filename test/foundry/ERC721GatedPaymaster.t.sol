...
import "src/ERC721GatedPaymaster.sol";
...
contract ERC721GatedPaymasterTest is Test {
  EntryPoint entryPoint;
  SimpleAccountFactory accountFactory;
  ERC721GatedPaymaster paymaster;
  ...
  function setUp() external {
    ...
    paymaster = new ERC721GatedPaymaster(entryPoint, address(nft));
  ...
  function testDeploy() external {
    vm.startPrank(paymasterOperator);
    ERC721GatedPaymaster testArtifact = new ERC721GatedPaymaster(entryPoint, address(nft));
    vm.stopPrank();
    assertEq(address(testArtifact.tokenContract()), address(nft));
  ...
  function testCall() external {
    ...
    (UserOperation memory op, uint256 prefund) = fillUserOp(account, userKey, address(0), 0, "");
  ...
  function testPaymasterFailed() external {
    ...
    vm.prank(testUser);
    SimpleAccount testAccount = accountFactory.createAccount(testUser, 0);
    vm.deal(address(testAccount), 1e18);
    (UserOperation memory op, uint256 prefund) = fillUserOp(testAccount, userKey, address(0), 0, "");
    op.paymasterAndData = abi.encodePacked(address(paymaster));
